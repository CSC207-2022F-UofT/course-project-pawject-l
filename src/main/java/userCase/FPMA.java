...
public class FPMA implements FPMARequestModel {
    ...
    public Pet[] getPossibleCandidates(Pet userPet) {
        ...
        float[] location = getLocation(userPetAttributes.getLatitude(), userPetAttributes.getLongitude()); //Initializes float list containing the coordinates of the user
        float preferredDistance = userPetAttributes.getPreferredProximity(); //Initializes float object containing the preferred proximity of the user
        ...
        while (count < 19) {
            ...
            if (!userPet.getDislikes().contains(candidate)) { //Checks if user has already disliked candidate
                if (!candidate.getDislikes().contains(userPet)) { //Checks if candidate already disliked user
                    if (getDistance(getLocation(candidatePetAttributes.getLatitude(), candidatePetAttributes.getLongitude()), location) < preferredDistance) { //Checks if candidates location satisfies users preferred proximity
